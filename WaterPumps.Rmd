...
```{r}
#Libraries
library_list = list('ggplot2','glmnet','ggmap','reshape2', 'randomForest')
...
KeepVars = setNames(data.frame(sapply(WaterPumps[,which(sapply(WaterPumps, is.character))], function(x){if(length(unique(x))>25){FALSE} else {TRUE}})),
                    c('factorlevels'))
...
RandomForest.mod = randomForest(status_group~., data=WaterPumps[train,], importance=TRUE)
