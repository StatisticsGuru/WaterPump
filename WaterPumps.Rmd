...
allLevels = unique(train.y);

for(i in 1) {
# train.1V1x = as.data.frame(train.x)
# dmtrain.x = data.matrix(train.x)
# valid.1V1.x = as.data.frame(valid.x)
# dmvalid.x = data.matrix(valid.x)

train.1V1.y = train.y 
train.1V1.y[train.y != allLevels[i]] = 'OTHER'
train.1V1.y = data.frame(Response = factor(train.1V1.y))

# train.1V1.y = as.data.frame(train.1V1.y)
# dmtrain.1V1.y = data.matrix(train.1V1.y)

valid.1V1.y = valid.y 
valid.1V1.y[valid.y != allLevels[i]] = 'OTHER'
# valid.1V1.y = as.data.frame(valid.1V1.y)
# dmvalid.1V1.y = data.matrix(valid.1V1.y)


fit.glmnet.cr <- glmnet.cr(train.x, train.1V1.y, alpha = 1)
...
#lassomulti.predicted = predict(fit.glmnet.cr,newx=valid.x)

# confusionMatrix(factor(lassomulti.fitted$class, levels=levels(train.y)), train.y)
# confusionMatrix(factor(lassomulti.predicted$class[,32], levels=levels(valid.y)), valid.y)
...
```
>>>>>>> 02c8093a87f9114defd6244846bd1b8293407031
