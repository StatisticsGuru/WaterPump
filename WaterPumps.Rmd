...
xgboost.test.x = xgboost.modelmatrix[valid,]
xgboost.test.y = xgboost.data$response[valid]
...
set.seed(37)
cv_best_model <- xgboost(data=train_matrix, params=best_param, nrounds=1000, nthread=6)

train_pred <- predict(cv_best_model, newdata = train_matrix)

train_prediction = data.frame(matrix(train_pred, ncol = no_of_classes, nrow=length(train_pred)/no_of_classes)) %>% mutate( max_prob = max.col(., "last"))
train_prediction['label']= xgboost.data[train,]$response + 1

ConfTrainXGBoost = confusionMatrix(factor(train_prediction$label), factor(train_prediction$max_prob))

test_pred <- predict(cv_best_model, newdata = test_matrix)
test_prediction <- matrix(test_pred, ncol = no_of_classes, nrow = length(test_pred)/no_of_classes) %>% t() %>% data.frame() %>%
  mutate(label = xgboost.data[valid,]$response + 1,max_prob = max.col(., "last"))


ConfTestXGBoost = confusionMatrix(factor(test_prediction$label),
...
By contrast, in the test set, the prediction accuracy was only `r round(ConfTestXGBoost$overall["Accuracy"][[1]],3)*100`%. 
